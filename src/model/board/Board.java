...
public class Board extends JLayeredPane implements GameConstants
{
	...
	public Board(int boardWidth, int boardHeight)
	{
		...
		makeHexGrid(boardWidth/2, boardHeight/2, 5, 0);
	}
	...
	private void makeHexGrid(int origin_x, int origin_y, int size, int padding)
	{
		...
		{
			int column = size - Math.abs(i - half);

			for (int h = -1; h < column+1; h++)
			{
				int x = (int) (origin_x + xOffset * (h * 2 + 1 - column));
				int y = (int) (origin_y + yOffset * (i - half) * 3);

				Tile tile = new Tile(x, y);
				tile.setResourceTile(ResourceTile.WATER);

				boardTiles.add(i, h, tile);
			}
		}
		//Sets the rest of the tiles
	...
	private void setResources(int playerNumber)
	{
		...
		if (playerNumber <= 4)
		{
			...
			resources.add(ResourceTile.WOOD);
			resources.add(ResourceTile.WOOD);
			resources.add(ResourceTile.WOOL);
			resources.add(ResourceTile.WOOL);
			resources.add(ResourceTile.WOOL);
			resources.add(ResourceTile.WOOL);
	...
	// PLAYERS | SHEEP(2:1) | WOOD(2:1) | CLAY(2:1) | ORE(2:1) | WHEAT(2:1) |
	// ANYTHING(3:1)
	// 3-4 | 1 | 1 | 1 | 1 | 1 | 4
	// 5-6 | 2 | 1 | 1 | 1 | 1 | 5
